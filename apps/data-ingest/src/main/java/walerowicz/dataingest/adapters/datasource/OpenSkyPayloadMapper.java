    ...
    private static StateVector map(Object[] state) {
        try {
            return StateVector.builder()
                    ...
                    .longitude(castToNumber(state[5], Float.class))
                    .latitude(castToNumber(state[6], Float.class))
                    .barometricAltitude(castToNumber(state[7], Float.class))
                    .onGround(cast(state[8], Boolean.class))
                    .velocity(castToNumber(state[9], Float.class))
                    .trueTrack(castToNumber(state[10], Float.class))
                    .verticalRate(castToNumber(state[11], Float.class))
                    // state[12] - sensors array omitted on purpose
                    .geometricAltitude(castToNumber(state[13], Float.class))
